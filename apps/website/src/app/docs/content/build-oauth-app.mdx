...
description: Create applications that integrate with CashPanel using OAuth 2.0.
section: developer
order: 2
---

Build applications that let users connect their CashPanel accounts. OAuth apps can access financial data on behalf of users, enabling integrations, automations, and custom tools.

## What you can build

With OAuth, you can build:

- **Browser extensions** that display CashPanel data
- **Mobile apps** with CashPanel integration
...
- **Public apps** listed in the CashPanel app directory

## How OAuth works

OAuth 2.0 lets your app request access to a user's CashPanel data without handling their password:

1. Your app redirects users to CashPanel's authorization page
2. Users log in and approve the requested permissions
3. CashPanel redirects back with an authorization code
4. Your app exchanges the code for access tokens
5. Use access tokens to call the CashPanel API
...
```
https://app.cashpanel.io/oauth/authorize?
  response_type=code&
...
After the user authorizes, CashPanel redirects to your `redirect_uri`:
...
```bash
curl -X POST https://api.cashpanel.io/v1/oauth/token \
...
```typescript
import { CashPanel } from "@cashpanel-ai/sdk";
...
// List transactions
const transactions = await cashpanel.transactions.list({
  pageSize: 50,
});

// Get invoices
const invoices = await cashpanel.invoices.list({
  pageSize: 20,
});

// Get financial metrics
const revenue = await cashpanel.metrics.revenue({
  from: "2024-01-01",
  to: "2024-12-31",
});
```
...
```bash
curl -X POST https://api.cashpanel.io/v1/oauth/revoke \
  -H "Content-Type: application/json" \
  -d '{
    "token": "mid_at_xxxxx",
    "client_id": "YOUR_CLIENT_ID",
...
const CLIENT_ID = process.env.CASHPANEL_CLIENT_ID!;
const CLIENT_SECRET = process.env.CASHPANEL_CLIENT_SECRET!;
...
app.get("/connect", (req, res) => {
  ...
  const authUrl = new URL("https://app.cashpanel.io/oauth/authorize");
...
app.get("/callback", async (req, res) => {
  ...
  const response = await fetch("https://api.cashpanel.io/v1/oauth/token", {
    method: "POST",
...
app.get("/transactions", async (req, res) => {
  ...
  const result = await cashpanel.transactions.list({
    pageSize: 50,
  });
