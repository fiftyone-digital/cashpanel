...
description: How the transaction export flow works in CashPanel.
section: export
order: 1
---

CashPanel tracks each transaction through a clear lifecycle so you always know what needs attention and what's been exported.
...
| **Receipt found** | CashPanel found a possible receipt match. Confirm or dismiss it. |
...
2. CashPanel finds a matching receipt automatically (status: **Receipt found**)
...
| **Mark exported** | Manually marks as exported (use only if exported outside CashPanel) |
