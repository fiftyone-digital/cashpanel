...
description: Technical reference for CashPanel OAuth 2.0 endpoints.
section: developer
order: 4
---

Complete technical reference for CashPanel's OAuth 2.0 implementation. These endpoints follow the [OAuth 2.0 RFC 6749](https://tools.ietf.org/html/rfc6749) and [PKCE RFC 7636](https://tools.ietf.org/html/rfc7636) specifications.

## Base URLs

| Environment | URL |
|-------------|-----|
| Authorization | `https://app.cashpanel.io/oauth` |
| Token & API | `https://api.cashpanel.io/v1` |
...
```
GET https://app.cashpanel.io/oauth/authorize
```
...
```
POST https://api.cashpanel.io/v1/oauth/token
```
...
```
POST https://api.cashpanel.io/v1/oauth/revoke
```
...
Once you have an access token, use the CashPanel SDK for API requests:
...
const profit = await cashpanel.metrics.profit({
  from: "2024-01-01",
  to: "2024-12-31",
});
```

See the [SDK documentation](https://github.com/cashpanel-ai/cashpanel-ts) for all available methods.
