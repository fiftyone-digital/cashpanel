...
description: How CashPanel automatically matches receipts and invoices to the right transactions, and what to expect in daily use.
section: inbox
order: 1
---

CashPanel captures receipts and invoices, then matches them to your bank transactions so reconciliation takes minutes instead of hours.
...
2. CashPanel extracts merchant, amount, date, and document details
...
CashPanel uses multiple signals together:
...
- a suggestion when CashPanel is likely right but wants confirmation,
...
CashPanel is highly confident this is correct and attaches the receipt automatically.
...
CashPanel thinks this is likely correct, but asks you to confirm.

Review and either:
- **Confirm**: Receipt is attached and matching gets smarter for similar cases
- **Reject**: CashPanel records this as negative feedback
...
CashPanel continuously improves through your team's actions:

### Merchant pattern learning

When your team repeatedly confirms the same merchant pairings, CashPanel starts treating those as safer and faster to match.
...
When you reject a match, CashPanel remembers that pair and avoids repeating it.

### Post-match learning

Even after confirmation, unmatching later is treated as negative feedback so the system avoids repeating similar mistakes.

## Getting receipts into CashPanel

### Email forwarding

Every CashPanel account has a unique inbox email address:

1. Go to [Inbox → Settings](https://app.cashpanel.io/inbox/settings)
...
4. CashPanel scans for receipts and invoices automatically
...
1. Connect your Slack workspace in [Inbox → Settings](https://app.cashpanel.io/inbox/settings)
2. Share any file to the connected channel
3. It appears in your CashPanel inbox automatically
...
1. Go to [Inbox](https://app.cashpanel.io/inbox)
...
The purchase might be on an account you haven't connected. Check your connected accounts in [Settings → Accounts](https://app.cashpanel.io/settings/accounts).
...
2. Manually confirm a few correct matches so CashPanel learns your pattern
...
- Connected email access is read-only—CashPanel can't send emails
